...
const SAMPLE = {
  '基礎理論': [
    ...
    {id:'B0161', term:'木構造と2分探索木', def:'データを親子関係でつないだ構造。2分探索木は「左の子＜親＜右の子」を保ち、探索・挿入をO(log n)で行える（偏ると最悪O(n)）。根から葉までの深さを木の高さという。'},
    {id:'B0162', term:'待ち行列理論（M/M/1）', def:'到着と処理がランダムな窓口1つのモデル。利用率ρ＝到着率÷処理率とすると、平均待ち時間は平均処理時間×ρ÷(1−ρ)。ρが1に近づくと待ち時間は急激に伸びる。'},
    {id:'B0163', term:'逆ポーランド記法（後置記法）', def:'演算子を被演算子の後ろに書く記法。A＋B は AB＋ となる。括弧が不要で、スタックを使って左から順に計算できるためコンパイラで使われる。'}
  ],
  'コンピュータ構成要素・ハードウェア': [
    ...
    {id:'B0164', term:'RISCとCISC', def:'RISCは命令を単純・固定長にそろえてパイプラインを効きやすくした方式。CISCは1命令で複雑な処理を行う方式。RISCは命令数が増えるが1命令あたりが速い。'},
    {id:'B0165', term:'メモリインタリーブ', def:'主記憶を複数のバンクに分け、連続するアドレスを別バンクに割り当てて並行アクセスする高速化技法。1つのバンクが応答している間に次のバンクを動かせる。'}
  ],
  'システム構成要素・信頼性設計': [
    ...
    {id:'B0166', term:'シンクライアントとVDI', def:'端末側に資料を残さず、サーバ側で処理と保管を行う方式。VDIはサーバ上の仮想デスクトップを端末から操作する。端末紛失時の情報漏えいを防げる。'},
    {id:'B0167', term:'エッジコンピューティング', def:'処理をクラウドに集約せず、利用者や機器の近くで行う方式。通信の遅延と回線の負荷を抑えられる。IoT機器の増加に伴い重要度が増している。'}
  ],
  'ソフトウェア・OS': [
    ...
    {id:'B0168', term:'カーネルとシェル', def:'カーネルはハードウェアやプロセスを直接管理するOSの中核。シェルは利用者の入力を解釈してカーネルに橋渡しする外殻。利用者が直接触れるのはシェル。'},
    {id:'B0169', term:'ガーベジコレクション', def:'どこからも参照されなくなったメモリ領域を自動で回収する仕組み。解放漏れ（メモリリーク）を防げるが、回収処理が動く間は処理が一時停止することがある。'}
  ],
  'ヒューマンインタフェース・マルチメディア': [
    ...
    {id:'B0170', term:'コーデックと主な形式', def:'音声・映像を符号化／復号する方式。MP3・AACは音声、MPEG-2・H.264は動画の代表例。JPEGは不可逆、PNG・GIFは可逆で、用途に応じて使い分ける。'}
  ],
  'データベース': [
    ...
    {id:'B0171', term:'関係代数（射影・選択・結合）', def:'選択は行を絞る操作、射影は列を取り出す操作、結合は複数の表を共通の項目でつなぐ操作。SQLのWHERE・SELECT・JOINにそれぞれ対応する。'},
    {id:'B0172', term:'トランザクションの分離レベル', def:'同時実行の厳しさの段階。緩いとダーティリード・反復不能読取り・ファントムリードが起こりうる。厳しくすると整合性は上がるが並行性（性能）は下がる。'}
  ],
  'ネットワーク': [
    ...
    {id:'B0173', term:'IPv6', def:'IPv4のアドレス枯渇に対応した128ビットのアドレス体系。16進数を4桁ずつコロンで区切って表記する。IPv4と直接互換はなく、共存にはトンネリングなどを使う。'},
    {id:'B0174', term:'無線LANの規格とWPA', def:'IEEE 802.11 系の規格で、a/g/n/ac/ax と世代が進むほど高速。暗号化はWEPが危殆化しており、現在はWPA2・WPA3を使う。SSIDは無線LANの識別名。'}
  ],
  'セキュリティ': [
    ...
    {id:'B0175', term:'脆弱性診断とペネトレーションテスト', def:'脆弱性診断は既知の弱点が無いかを網羅的に調べる。ペネトレーションテストは攻撃者の立場で実際に侵入を試み、被害がどこまで及ぶかを確かめる。目的と深さが違う。'}
  ],
  'システム開発技術': [
  ...
  'プロジェクトマネジメント': [
    ...
    {id:'B0176', term:'レビューの種類', def:'ウォークスルーは作成者が説明する非公式なレビュー。インスペクションはモデレータが進行する公式なレビュー。早い工程で欠陥を見つけるほど修正費用は小さい。'},
    {id:'B0177', term:'クラッシングとファストトラッキング', def:'日程を短縮する技法。クラッシングは要員や資源を投入して期間を縮める（コストが増える）。ファストトラッキングは順に行う予定の作業を一部並行させる（手戻りのリスクが増える）。どちらもクリティカルパス上の作業に対して行う。'},
    {id:'B0178', term:'見積り手法の使い分け', def:'類推見積りは過去の類似案件から概算する（企画段階向き）。ボトムアップ見積りはWBSの作業を積み上げる（精度は高いが後工程向き）。ファンクションポイント法は機能の数と複雑さから規模を測る。'},
    {id:'B0179', term:'PMBOKの考え方', def:'プロジェクトマネジメントの知識体系。スコープ・スケジュール・コスト・品質・資源・リスクなどの領域ごとに、計画から監視・終結までの手順を体系化している。'}
  ],
  'サービスマネジメント・システム監査': [
    ...
    {id:'B0180', term:'サービスレベル管理（SLM）', def:'SLAで合意した水準を維持できているか継続的に測定し、報告・改善するPDCAの活動。SLAは合意そのもの、SLMはそれを回す運用という関係。'},
    {id:'B0181', term:'バックアップの方式', def:'フルは全体、差分は前回のフル以降の変更分、増分は前回のバックアップ以降の変更分を取る。増分は取得が速いが復元にすべての世代が必要で時間がかかる。'},
    {id:'B0182', term:'システム監査の手順', def:'監査計画の立案→予備調査→本調査→評価・結論→報告・フォローアップの順に進む。監査人は被監査部門から独立している必要がある。'}
  ],
  '経営戦略・システム戦略・法務': [
    ...
    {id:'B0183', term:'3C分析', def:'市場・顧客（Customer）、競合（Competitor）、自社（Company）の三つの視点から事業環境を整理する枠組み。自社の強みが顧客の求めと合っているかを確かめる。'},
    {id:'B0184', term:'ファイブフォース分析', def:'業界の収益性を「新規参入の脅威・代替品の脅威・買い手の交渉力・売り手の交渉力・既存企業間の対抗」の5つの力で分析する手法。'},
    {id:'B0185', term:'アンゾフの成長マトリクス', def:'製品（既存・新規）と市場（既存・新規）の組合せで成長戦略を4つに分ける。市場浸透・新製品開発・新市場開拓・多角化。多角化が最もリスクが高い。'},
    {id:'B0186', term:'マーケティングミックス（4Pと4C）', def:'売り手側の4PはProduct・Price・Place・Promotion。買い手側の4CはCustomer Value・Cost・Convenience・Communication。両者は対応関係にある。'},
    {id:'B0187', term:'プロダクトライフサイクル', def:'導入期・成長期・成熟期・衰退期の4段階で製品の売上と利益の推移をとらえる考え方。段階ごとに投資や価格の戦略を変える。'},
    {id:'B0188', term:'KGI・KPI・CSF', def:'KGIは最終的な目標値、KPIはその達成度を測る中間指標、CSFは目標達成に不可欠な要因。CSFを見極めてKPIを設定し、KGIにつなげる。'},
    {id:'B0189', term:'バランススコアカード（BSC）', def:'財務・顧客・業務プロセス・学習と成長の4つの視点から戦略を評価する手法。財務だけに偏らず、その成果を生む活動まで管理できる。'},
    {id:'B0190', term:'エンタープライズアーキテクチャ（EA）', def:'組織の業務とシステムの全体像を、ビジネス・データ・アプリケーション・技術の4つの体系で整理し、現状（As-Is）と理想（To-Be）の差を埋めていく手法。'},
    {id:'B0191', term:'共通フレーム（SLCP）', def:'システムの企画から開発・運用・保守までの作業内容と用語を定めた共通の物差し。発注者と受注者の間で作業範囲の解釈がずれるのを防ぐ。'},
    {id:'B0192', term:'業務モデリング（DFDとBPMN）', def:'DFDはデータの流れに着目して業務を図式化する。BPMNは業務の流れを標準記法で表す。E-R図がデータの構造を表すのに対し、こちらは処理の流れを表す。'},
    {id:'B0193', term:'DXとIoT', def:'IoTは機器をネットワークにつなぎデータを集める仕組み。DXはデジタル技術で業務や事業のあり方そのものを作り変える取組みで、単なる電子化とは区別される。'},
    {id:'B0194', term:'ビッグデータとデータサイエンス', def:'量・種類・発生速度の3つのVで特徴づけられる大量データと、その分析から価値を引き出す取組み。分析結果を意思決定に結び付けることが目的。'},
    {id:'B0195', term:'クラウドの3形態', def:'SaaSは完成したアプリを使う、PaaSはOS・ミドルウェアまで借りてアプリを開発する、IaaSは仮想サーバなど基盤だけを借りる。利用者が管理する範囲が異なる。'},
    {id:'B0196', term:'利益の段階', def:'売上総利益＝売上高−売上原価。営業利益＝売上総利益−販売費及び一般管理費。経常利益＝営業利益＋営業外収益−営業外費用。段階ごとに何を引くかが問われる。'},
    {id:'B0197', term:'棚卸資産の評価', def:'先入先出法は先に仕入れた分から払い出したとみなす方法で、物価上昇時は期末在庫の評価額が高くなる。移動平均法は仕入れのたびに平均単価を計算し直す。'},
    {id:'B0198', term:'下請法と独占禁止法', def:'独占禁止法は公正な競争を妨げる行為を禁じる法律。下請法はその特別法で、親事業者による代金の減額や支払遅延など、下請事業者への不当な扱いを禁じる。'},
    {id:'B0199', term:'製造物責任法（PL法）', def:'製造物の欠陥で生命・身体・財産に被害が生じた場合、製造業者の過失を証明しなくても損害賠償を請求できる法律。組み込みソフトを含む製造物が対象になる。'},
    {id:'B0200', term:'コーポレートガバナンスと内部統制', def:'コーポレートガバナンスは経営が適切に行われるよう監視する仕組み。内部統制は業務の有効性・報告の信頼性・法令順守・資産の保全を目的に社内で整備する仕組み。'}
  ],
};
...
const CHEATSHEETS = [
  ...
  {id:'algo', icon:'code', title:'アルゴリズムとデータ構造の使い分け（科目B）',
   headers:['用途', '選ぶもの', '計算量の目安', '科目Bでの見分け方'],
   rows:[
     ['並びの中から目的の値を探す（未整列）', '線形探索', 'O(n)', '先頭から順に比較する for が1本。見つかったら return する形'],
     ['並びの中から目的の値を探す（整列済み）', '二分探索', 'O(log n)', 'lo と hi を持ち、中央 mid と比べて範囲を半分にする while'],
     ['後から入れたものを先に取り出す', 'スタック（LIFO）', '出し入れ O(1)', '末尾に追加し、末尾から取り出す。関数呼出しや式の評価に対応'],
     ['先に入れたものを先に取り出す', 'キュー（FIFO）', '出し入れ O(1)', '先頭から取り出す。印刷待ちや順番待ちの処理に対応'],
     ['キーから一発で値を引く', 'ハッシュ法', '平均 O(1)・最悪 O(n)', 'キーを数値に変換して位置を決める。衝突時はチェーン法など'],
     ['階層や大小関係で整理する', '木・2分探索木', '探索 O(log n)（偏ると O(n)）', '左の子＜親＜右の子。再帰で左右をたどる形が多い'],
     ['小さい順に並べ替える（単純）', 'バブル・選択・挿入ソート', 'O(n²)', '二重ループ。内側で隣どうしを比べるか、最小値の位置を覚える'],
     ['小さい順に並べ替える（高速）', 'クイック・マージソート', '平均 O(n log n)', '分割して再帰的に処理し、あとで併合する形'],
   ]},
  {id:'kamokub', icon:'target', title:'科目Bのつまずきポイントと読み方',
   headers:['つまずくところ', '正しい読み方', '確認のしかた'],
   rows:[
     ['配列の要素番号', '1から始まる。data[0] は存在しない', '問題文の「要素番号は 1 から始まる」を必ず確認する'],
     ['for の制御記述', '「i を 1 から n まで 1 ずつ増やす」＝ i は n を含む', '終了値を含むかどうかで回数が1回ずれる'],
     ['前判定と後判定', 'while は0回で終わることがある。do 〜 while は必ず1回は実行', '初回に条件が偽になるケースを試す'],
     ['空欄が複数ある問題', '組合せの解答群なので、1か所でも合わなければ不正解', '確実に分かる空欄から絞り、残りを消去法で決める'],
     ['トレースの進め方', '変数を横に並べた表を書き、1行ずつ値を更新する', '頭の中で追わない。3〜4行書けば規則が見える'],
     ['境界値', '0件・1件・最後の要素で確かめる', '「n個目まで」と「n−1個目まで」の違いで答えが変わる'],
     ['再帰', '基底（終了条件）から先に確かめ、小さい値で手を動かす', 'f(1)、f(2) を求めてから f(5) を組み立てる'],
     ['セキュリティ4問', '擬似言語でも問われるのは仕組みの理解', '「許可リストか拒否リストか」「認証と認可の順序」を意識する'],
   ]},
  {id:'dbsql', icon:'database', title:'正規化とSQLの読み解き',
   headers:['項目', '内容', 'つまずきやすい点'],
   rows:[
     ['第1正規形', '繰返し項目を別の行に分け、表を長方形にする', '1つのセルに複数の値が入っていたら未正規形'],
     ['第2正規形', '主キーの一部だけで決まる項目を別表に分ける', '主キーが単一項目なら第2正規形は自動的に満たす'],
     ['第3正規形', '主キー以外の項目で決まる項目を別表に分ける', '「推移的関数従属を除く」と表現される'],
     ['SQLの実行順序', 'FROM → WHERE → GROUP BY → HAVING → SELECT → ORDER BY', 'SELECT の別名は WHERE では使えない（まだ決まっていない）'],
     ['WHERE と HAVING', 'WHERE は集計前の行、HAVING は集計後のグループに条件を付ける', 'COUNT や SUM の条件は HAVING に書く'],
     ['内部結合と外部結合', '内部結合は両方にある行だけ。外部結合は片方に無くても残す', '「該当が無い行も出したい」なら外部結合'],
     ['主キーと外部キー', '主キーは行を一意に決める。外部キーは他の表の主キーを参照する', '参照整合性：参照先に無い値は入れられない'],
     ['トランザクション', 'ACID特性。コミットで確定、ロールバックで取消し', '障害復旧はロールフォワード（前進復帰）と対で覚える'],
   ]},
  {id:'crypto', icon:'lock', title:'暗号・認証技術の使い分け',
   headers:['やりたいこと', '使う技術', '鍵の使い方', '補足'],
   rows:[
     ['内容を第三者に読ませない（大量データ）', '共通鍵暗号（AESなど）', '送信者と受信者が同じ鍵を共有', '処理は速いが、鍵の受け渡しが課題'],
     ['鍵の受け渡しを安全にする', '公開鍵暗号（RSAなど）', '受信者の公開鍵で暗号化し、秘密鍵で復号', '処理は遅い。共通鍵の受け渡しに使う'],
     ['両方の良いところを取る', 'ハイブリッド暗号方式', '本文は共通鍵、その鍵を公開鍵で暗号化', '実際のTLSはこの形'],
     ['改ざんされていないことを示す', 'ハッシュ関数（SHA-256など）', '鍵は使わない', '同じ入力なら同じ値。元には戻せない'],
     ['誰が送ったかを証明する', 'デジタル署名', '送信者の秘密鍵で署名し、公開鍵で検証', '暗号化と鍵の使い方が逆になる点が頻出'],
     ['公開鍵が本物だと保証する', 'PKIと認証局（CA）', 'CAがデジタル証明書を発行する', '公開鍵の持ち主が本人かを第三者が保証する'],
     ['本人確認を厳しくする', '多要素認証', '知識・所持・生体のうち2つ以上を組み合わせる', 'パスワード2つは多要素にならない'],
     ['パスワードを保存する', 'ハッシュ化＋ソルト', '平文では保存しない', '漏えいしても元のパスワードを求めにくくする'],
   ]}
];
