...
// クラウド3形態の管理範囲。ストラテジ系（システム戦略）で使う
const FIG_CLOUD_FE = `<svg viewBox="0 0 400 200" width="400" role="img" aria-label="SaaSとPaaSとIaaSで利用者と事業者の管理範囲を比べた図">
  <g fill="none" stroke="currentColor" stroke-width="1.3" font-size="10.5">
    ${['アプリケーション','ミドルウェア','OS','ハードウェア'].map(function(name, r){
      return '<text x="6" y="' + (58 + r*32) + '" fill="currentColor" stroke="none">' + name + '</text>';
    }).join('')}
    ${[['SaaS',[0,0,0,0]],['PaaS',[1,0,0,0]],['IaaS',[1,1,1,0]]].map(function(col, c){
      const x = 118 + c*92;
      return '<text x="' + (x + 26) + '" y="30" font-size="11.5" fill="currentColor" stroke="none">' + col[0] + '</text>' +
        col[1].map(function(mine, r){
          return '<rect x="' + x + '" y="' + (40 + r*32) + '" width="78" height="26" rx="3" ' +
                 'stroke="' + (mine ? 'var(--orange)' : 'var(--dim)') + '"/>' +
                 '<text x="' + (x + 12) + '" y="' + (57 + r*32) + '" fill="' + (mine ? 'var(--orange)' : 'var(--dim)') + '" stroke="none">' +
                 (mine ? '利用者が管理' : '事業者が管理') + '</text>';
        }).join('');
    }).join('')}
    <text x="6" y="188" font-size="10.5" fill="var(--orange)" stroke="none">オレンジ＝利用者が管理する範囲</text>
  </g>
</svg>`;
...
/* ── 2026-08-25 追加分（S048〜S057）──────────────────────────
   ストラテジ系の在庫が17問で、模試の必要数12問に対して余裕が乏しかったため増補。
   法務分野は細かい数値や条文番号に踏み込まず、制度の趣旨と用語の対応を問う形にしている
   （17-5節の方針）。 */

{id:'S048',c:'strategy',d:1,
 q:'新規事業の検討にあたり、市場（顧客）・競合・自社の三つの視点から状況を整理したいと考えています。適したフレームワークはどれですか？',
 o:['3C分析','PPM','バリューチェーン分析','ファイブフォース分析'],a:0,
 e:'3C分析はCustomer（市場・顧客）、Competitor（競合）、Company（自社）の三つの視点から事業環境を整理する枠組みです。「PPM」は複数の事業を市場成長率と市場占有率で分類し、資源配分を決める手法で、三つの視点という切り口ではありません。「バリューチェーン分析」は自社の活動を機能ごとに分解してどこで価値が生まれるかを見る手法で、競合や顧客を並べて分析するものではありません。「ファイブフォース分析」は業界の競争要因を五つの力で分析する手法です。'},

{id:'S049',c:'strategy',d:2,
 q:'自社の強みが発揮できない定型的な間接業務について、外部の専門事業者に任せて経営資源を主力事業へ集中させたいと考えています。この考え方はどれですか？',
 o:['コアコンピタンスへの集中','垂直統合による内製化の推進','事業のドメイン拡大による多角化','規模の経済によるコスト低減'],a:0,
 e:'自社の中核的な強み（コアコンピタンス）に資源を集中し、それ以外を外部に委託するのがアウトソーシングの基本的な考え方です。「垂直統合による内製化の推進」は逆に工程を自社に取り込む戦略で、外部委託とは方向が反対です。「事業のドメイン拡大による多角化」は事業領域を広げる戦略で、間接業務の委託とは目的が異なります。「規模の経済によるコスト低減」は生産量を増やして単位あたりのコストを下げる考え方で、業務を外部に任せる判断そのものを説明しません。'},

{id:'S050',c:'strategy',d:2,
 q:'自社で預かっている顧客の個人データを、分析のために外部の事業者へ渡すことになりました。個人情報の保護に関する法制度を踏まえた対応として、原則として必要になるものはどれですか？',
 o:['第三者へ提供することについて、あらかじめ本人の同意を得る','データの件数を所管官庁へ届け出る','提供先の事業者と資本関係を結ぶ','提供するデータを暗号化すれば同意は不要になる'],a:0,
 e:'個人データを第三者へ提供する場合は、原則としてあらかじめ本人の同意を得る必要があります。「データの件数を所管官庁へ届け出る」は制度上求められている手続ではありません。「提供先の事業者と資本関係を結ぶ」も要件ではなく、資本関係があっても別法人であれば第三者提供にあたります。「提供するデータを暗号化すれば同意は不要になる」も誤りで、暗号化は安全管理の手段であって、同意の代わりにはなりません。'},

{id:'S051',c:'strategy',d:2,
 q:'A社がB社にソフトウェアの開発を請負契約で委託しました。契約書に著作権の扱いについて特段の定めがない場合、完成したプログラムの著作権は原則としてどちらに帰属しますか？',
 o:['実際に開発した受注者（B社）','費用を負担した発注者（A社）','両社の共有となり、単独では行使できない','公表と同時に消滅し、どちらにも帰属しない'],a:0,
 e:'著作権は原則として著作物を創作した者に発生するため、特約がなければ受注者（B社）に帰属します。発注者が権利を得たい場合は契約で移転を定める必要があります。「費用を負担した発注者（A社）」は費用の負担と著作権の帰属を混同したもので、特約がなければ移転しません。「両社の共有となり、単独では行使できない」は共同で創作した場合の扱いで、委託しただけでは共有になりません。「公表と同時に消滅し、どちらにも帰属しない」は誤りで、著作権は公表によって消滅しません。'},

{id:'S052',c:'strategy',d:3,
 q:'自社の製造ノウハウを不正競争防止法上の「営業秘密」として保護したいと考えています。満たす必要がある要件の組合せはどれですか？',
 o:['秘密として管理されていること、有用であること、公然と知られていないこと','特許庁に出願していること、有用であること、公然と知られていないこと','秘密として管理されていること、文書化されていること、社外に公開していること','従業員と秘密保持契約を結んでいること、有用であること、特許を取得していること'],a:0,
 e:'営業秘密として保護されるには、秘密管理性・有用性・非公知性の三つを満たす必要があります。「特許庁に出願していること、有用であること、公然と知られていないこと」は特許の要件を混ぜたもので、出願すると内容が公開されるため非公知性を失い、営業秘密としては保護されなくなります。「秘密として管理されていること、文書化されていること、社外に公開していること」は、公開してしまえば非公知性を欠くため成立しません。「従業員と秘密保持契約を結んでいること、有用であること、特許を取得していること」の秘密保持契約は秘密管理の一手段にすぎず、特許の取得は要件ではありません。'},

{id:'S053',c:'strategy',d:3,
 q:'システム化投資の評価にあたり、投資額を何年で回収できるかという分かりやすさを重視して案を比較したいと考えています。適した評価手法はどれですか？',
 o:['回収期間法','正味現在価値法（NPV）','内部収益率法（IRR）','損益分岐点分析'],a:0,
 e:'回収期間法は、投資額を毎年の効果額で割るなどして回収までの年数を求める手法で、計算が簡単で説明しやすいのが特徴です。「正味現在価値法（NPV）」は将来の収入を現在価値に割り引いて評価する手法で、精度は高いものの割引率の設定が必要で分かりやすさでは劣ります。「内部収益率法（IRR）」は正味現在価値がゼロになる割引率を求める手法で、こちらも回収年数を直接は示しません。「損益分岐点分析」は売上と費用が釣り合う点を求める分析で、投資の回収年数を評価する手法ではありません。'},

{id:'S054',c:'strategy',d:2,
 q:'発注者と受注者の間で「要件定義」や「システム方式設計」といった作業の範囲や用語の解釈が食い違い、トラブルになることを防ぎたいと考えています。両者が共通の物差しとして参照すべきものはどれですか？',
 o:['共通フレーム','エンタープライズアーキテクチャ（EA）','サービスレベル合意書（SLA）','システム管理基準'],a:0,
 e:'共通フレームは、システム開発の作業内容と用語を発注者と受注者が同じ意味で使えるように定めた共通の枠組みです。「エンタープライズアーキテクチャ（EA）」は組織の業務とシステムの全体像を体系化して改善につなげる手法で、作業範囲の用語を定める目的のものではありません。「サービスレベル合意書（SLA）」は運用段階のサービス品質を数値で合意する文書です。「システム管理基準」はシステム管理のあるべき姿を示した監査の判断尺度で、開発工程の用語統一を目的としていません。'},

{id:'S055',c:'strategy',d:1,fig:FIG_CLOUD_FE,
 q:'図はクラウドサービスの3形態について、利用者と事業者のどちらが管理するかを比べたものです。OSやミドルウェアの管理は事業者に任せ、利用者はアプリケーションの開発・実行だけに専念したい場合、選ぶべき形態はどれですか？',
 o:['PaaS','SaaS','IaaS','オンプレミス'],a:0,
 e:'図のとおりPaaSは、OSやミドルウェアまでを事業者が管理し、利用者はその上でアプリケーションを開発・実行します。「SaaS」は完成した業務アプリケーションを利用する形態で、利用者が独自のアプリケーションを開発する余地がありません。「IaaS」は仮想サーバなどの基盤だけを借りる形態で、OSやミドルウェアの管理は利用者の役目として残ります。「オンプレミス」は自社で設備を保有する形態で、クラウドサービスの3形態には含まれません。'},

{id:'S056',c:'strategy',d:2,
 q:'システムの共同開発にあたり、相手企業へ開示する技術情報が目的外に使われたり第三者へ漏れたりすることを、契約によって防ぎたいと考えています。まず締結すべきものはどれですか？',
 o:['秘密保持契約（NDA）','ライセンス契約','保守契約','サービスレベル合意書（SLA）'],a:0,
 e:'秘密保持契約（NDA）は、開示した情報の利用目的を限定し、第三者への開示を禁じる契約で、情報を渡す前に結ぶのが基本です。「ライセンス契約」はソフトウェアなどの使用許諾の条件を定める契約で、開示情報の秘密保持を主目的とはしません。「保守契約」は納入後の維持管理の内容を定める契約です。「サービスレベル合意書（SLA）」は提供するサービスの品質水準を合意する文書であり、情報漏えいの防止を定めるものではありません。'},

{id:'S057',c:'strategy',d:2,
 q:'ある期の売上高が 2,400 万円、売上原価が 1,500 万円、販売費及び一般管理費が 600 万円でした。この期の営業利益はいくらですか？',
 o:['300万円','900万円','1,800万円','2,400万円'],a:0,
 e:'売上総利益は売上高－売上原価＝2,400－1,500＝900万円、営業利益はそこから販売費及び一般管理費を引いた900－600＝300万円です。「900万円」は売上総利益であり、販売費及び一般管理費を引く前の値です。「1,800万円」は売上高から販売費及び一般管理費だけを引いた値で、売上原価を差し引いていません。「2,400万円」は売上高そのものです。'},
];
